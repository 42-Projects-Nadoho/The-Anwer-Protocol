...
## Build
- [ ]  Project implementation language: The project is implemented in one of the allowed languages (C, C++, Rust, Go, or Zig).
- [ ]  Building tool targets: On a clean environment, the building tool provides targets/commands for `install dependencies`, `run-server`, `run-client`, `run-client-gui`, `lint`, `clean`.
  - [ ]  `install` target: install must complete without errors.
...
- [ ]  Abrupt client disconnect: Disconnect a client abruptly; the server must continue and remove the session.
...
## Combat System
- [ ]  Use `STATUS` command at the beginning: Players start with 100 HP.
...
- [ ]  Document combat mechanics: The combat mechanics design is documented in `README` with clear justification.
...
- [ ]  Quest types: At least 2 different quest types are implemented (fetch item, defeat NPC, deliver item).
- [ ]  Quest completion:
  - [ ]  Quest is validated
  - [ ]  Reward is granted.
- [ ]  NPCs without quests: Confirm NPCs without quests return `NO_QUEST_AVAILABLE` error.
- [ ]  Document quest mechanics: The quest progression mechanics are documented in `README` with implementation approach.
...
- [ ]  Abuse patterns: Abuse patterns (command flooding, rapid connections) are monitored and logged.
