...
|  | CLI client | Present |
|  | GUI client | Present |
|  | Static world data (YAML or JSON) | Present |
|  | README.md file at the root | Present |
|  | No unauthorised files are present | Directory is clean |
...
|  | Handle client disconnect | Disconnect a client while an event is being broadcast: the server removes the client cleanly and continues operating |
